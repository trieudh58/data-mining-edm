    ...
    with open("combine_rate_" + str(input_rate) + ".csv", 'w') as res:
        ...
        def combine(subject, fold):
            try:
                subject = str(subject)
                fold = str(fold)
                # with open("CF/prediction_all/prediction_" + subject + "_fold_" + fold + ".csv", 'rb') as cf_predict:
                with open("CF/prediction_all_no_following_sems/prediction_" + subject + "_fold_" + fold + ".csv", 'rb') as cf_predict:
                    ...
                    RATE = input_rate
                    # with open("SVM/predict_all/output_" + subject + "_" + fold, 'rb') as svm_predict:
                    with open("SVM/predict_no/output_" + subject + "_" + fold, 'rb') as svm_predict:
                        svm_reader = csv.reader(svm_predict, delimiter=';', quotechar='|')
                        with open("real/all/test_" + subject + "_fold_" + fold + ".csv", 'rb') as test:
